...
4.5. **Sister projects section** — collapsible `<details>` with table of all 14 Productions public repos. Auto-generated by `scripts/repo-bootstrap/promo_sync.py` (sub-project D). Block delimited by `<!-- BEGIN/END: AUTO-GENERATED-CROSS-PROMO -->`. Manual edits inside delimiters are reverted on next sync. Position: top of README, attention zone after badges, before Nav links + TOC. Scope: Productions public only (14 repos as of 2026-05-07).
...
## Package Registry README Sync

When a repo publishes to a package registry, README content MUST sync to that registry's display surface. Failure mode: registry shows "No description" / stale tagline / wrong link → silent UX regression.

### Per-registry mechanism table

| Registry | README delivery | Required field | Sync action |
|---|---|---|---|
| PyPI | upload at `uv publish` | `pyproject.toml [project] readme = "README.md"` | none extra |
| npm (npmjs.com) | auto from package root | `package.json` `repository` correct + `files` array includes `README.md` (default) | none extra |
| crates.io | upload at `cargo publish` | `Cargo.toml [package] readme = "README.md"` | none extra |
| pkg.go.dev | live-fetch GitHub | module path correct in `go.mod` | none extra |
| GHCR | auto-link GitHub repo | `LABEL org.opencontainers.image.source=https://github.com/<owner>/<repo>` in Dockerfile | verify in pre-commit |
| Docker Hub | requires API upload | n/a (no current repo uses Docker Hub) | rule: if Docker Hub adopted later, add CD step `docker.io/api/v2/repositories/<user>/<repo>/` PATCH |
| MCP Registry | upload `server.json` at publish | `server.json description` matches README tagline (line 3 of skret-style template) | CD job `verify-readme-sync` step `tagline-match` |
| Scoop bucket (`n24q02m/scoop-bucket`) | manifest in tap repo | manifest `description` + `homepage` synced from source repo at release | CD step in source repo: PR into tap repo updating manifest |
| Homebrew tap (`n24q02m/homebrew-tap`) | formula in tap repo | formula `desc` + `homepage` | same pattern as Scoop |
| Nix flake | `flake.nix meta.description` + `meta.homepage` | inline | verify in pre-commit |
| Aqua/mise | aqua-registry YAML | `description` + `homepage` | manual PR to aquaproj/aqua-registry on major release |

### Pre-commit lint (script `scripts/repo-bootstrap/verify_readme_sync.py` invoked from profile repo)

Per-language assertions:

- **Python**: `pyproject.toml` MUST have `[project] readme = "README.md"`
- **TS**: `package.json` MUST have `"repository"` field pointing to `https://github.com/n24q02m/<repo>.git`
- **Go**: Dockerfile MUST have `LABEL org.opencontainers.image.source=https://github.com/n24q02m/<repo>` (for GHCR)
- **All**: README.md MUST exist at root, MUST have non-empty tagline (line 3 of skret-style template)

### CD gate (`.github/workflows/cd.yml` job `verify-readme-sync`, runs before publish steps)

- Re-run pre-commit lint
- For MCP servers: parse `server.json description` + parse README.md tagline (line 3) → assert match (allow trailing punctuation difference only)
- For skret: assert tap-repo manifests have matching `description` (fetch from `n24q02m/scoop-bucket`, `n24q02m/homebrew-tap`)
- Block release if any check fails

### Anti-patterns

- Publish package without `[project].readme` field → user lands on PyPI page showing "No description"
- `server.json description = "TODO"` → MCP Registry catalog shows garbage entry
- Tap manifest `description` lags source repo (bumped source but forgot to update tap)
- Adopt Docker Hub without README upload step → Docker Hub page blank
...
Location: `n24q02m/n24q02m/scripts/repo-bootstrap/*.py` (Python 3.13+, pure stdlib). Pattern A reorg (Spec H) future-moves to `claude/scripts/repo-bootstrap/` but path stays stable for consumer composite actions until then.

Purpose: Idempotent rule enforcement — single source of truth for repo init + audit + retrofit, used by both local pre-commit hooks and CI workflows across all 17 public repos.

### Commands

| Command | Purpose | Idempotent? |
|---|---|---|
| `init.sh --lang=<python\|ts\|go\|rust\|monorepo> --tier=<1\|2> [--mcp]` | Scaffold new empty directory with full standard set from `claude/templates/` | yes (refuses if non-empty unless `--force`) |
| `audit.sh [--repo=<owner>/<name>]` | Run 6-medium GitHub-detail audit. Read-only. Outputs JSON/table/markdown report. Exit 0 if all pass, exit 1 if any fail | yes (read-only) |
| `apply.sh [--dry-run]` | Idempotent retrofit on existing repo. Three-way merge: existing vs template vs managed-region marker block | yes (re-run = noop on aligned files) |
| `verify.sh` | CI gate command: re-run audit + verify pre-commit hooks installed + verify CD workflow has required jobs. Exit non-zero if non-compliant | yes |
| `promo-sync.sh [--dry-run]` | Generate cross-promo block (sub-project B) + open PRs to all 14 Productions public repos. Block delimited by `<!-- BEGIN/END: AUTO-GENERATED-CROSS-PROMO -->` | yes (skip PR if no diff) |
| `verify-readme-sync.sh` | Per-language package registry README field check (called by pre-commit + CD job `verify-readme-sync`) | yes |

### CI/CD reuse — GitHub composite actions

Location: `n24q02m/n24q02m/.github/actions/repo-bootstrap-{audit,verify,promo-sync}/action.yml`
...
Native GitHub mechanism — no `git clone` step needed in consumer CI; GitHub Actions runtime fetches the action repo automatically.

### Idempotency strategy

- **Managed regions** (README cross-promo, ci.yml standard jobs, renovate.json shared rules): wrap content in `<!-- MANAGED-START: <key> -->` ... `<!-- MANAGED-END: <key> -->` markers; CLI rewrites only inside markers, preserves outside content.
- **Atomic-replace** (PR/issue templates, COC, LICENSE, SECURITY): overwrite if hash differs from canonical template; CLI prompts confirmation.
- **Compose-only** (`.mise.toml`, `.pre-commit-config.yaml`, `pyproject.toml`, `package.json`): TOML/YAML/JSON deep-key merge — insert template keys, preserve user keys, error on value conflict.
- **GitHub remote settings** (`gh repo edit`, `gh api`): fetch current state, PATCH delta only. `gh api repos/{repo}/rulesets` POST creates if absent, PUT updates if exists.

### 6-medium GitHub-detail audit scope

Full audit covered by `audit.sh`. All checks read-only via `gh api` / `gh repo view` / file system.

| Check group | Items |
|---|---|
| About panel | description (≤350 chars, matches README tagline) + homepage (matches `<repo>.n24q02m.com` for Tier 1) + ≥3 topics + social preview image uploaded (non-default) |
| Settings → General | default branch=main, merge methods=squash only, archived=false |
| Settings → Branches | Main ruleset applied, bypass list = repo admin only |
| Settings → Code security | CodeQL (public) or Semgrep (private), secret scanning enabled, push protection enabled, private vulnerability reporting enabled, Dependabot alerts enabled |
| Settings → Webhooks | Renovate app present, Codecov app (Tier 1), Cloudflare Pages (docs-site repos) |
| Issues/PRs | CODEOWNERS, ISSUE_TEMPLATE/{bug_report,feature_request}.md, PULL_REQUEST_TEMPLATE.md, best_practices.md, rulesets/main.json, standard labels (bug/enhancement/dependencies/documentation/security) |
| Discussions | enabled (Tier 1 public) |
| Community Standards | all green per GitHub `/community` tab |
| Root files | AGENTS + CHANGELOG + CONTRIBUTING + CODE_OF_CONDUCT + LICENSE + README + SECURITY + renovate.json + .skret.yaml + .mise.toml + .pre-commit-config.yaml + .pr_agent.toml |
| README compliance | tagline + 2-row badges + cross-promo block + ToC + (Tier 1) install matrix + comparison + sponsors + acknowledgments |

Output formats: `--format=json` (machine), `--format=table` (human default), `--format=markdown` (PR body).

### Anti-patterns

- Manual edit inside `<!-- MANAGED-* -->` regions → next CLI run reverts; rule: edit template in `claude/templates/`, not the regen target.
- Run `apply.sh` without `--dry-run` first on a repo with custom code outside managed regions.
- Skip `audit.sh` after `init.sh` → newly-init repo may miss check (e.g., CodeQL still default-disabled until first push triggers GH onboarding).
- Hardcode AWS account ID / Cloudflare account ID / SSM full paths in script templates → public repo leak via profile sync. Use `<placeholder>` + read from `.skret.yaml` / env at runtime.
...
- [ ] Package registry README sync: per-registry mechanism table satisfied (PyPI `[project].readme`, npm `repository`, MCP Registry `server.json description`, etc.)?
- [ ] Pre-commit `verify-readme-sync.sh` configured?
- [ ] CD `verify-readme-sync` job runs before publish steps?
- [ ] Cross-promo block (`<!-- BEGIN/END: AUTO-GENERATED-CROSS-PROMO -->`) present in Tier 1 README?
- [ ] GitHub-detail audit (6-medium scope) passes via `scripts/repo-bootstrap/audit.py`?
