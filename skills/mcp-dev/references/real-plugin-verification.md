...
- Test ít nhất:
  - 1 lệnh **ko cần upstream API** (vd `config status`, `help`) — verify daemon alive + tool routing OK.
  - 1 lệnh **cần upstream API** (vd `mnemo memory search`, `wet search`, `imagine generate`) — verify creds resolution + upstream call.
