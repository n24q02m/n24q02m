...
_SKIP_SUFFIXES = {
    ".lock", ".svg", ".png", ".jpg", ".jpeg", ".gif", ".webp", ".ico",
    ".pdf", ".zip", ".tar", ".gz", ".woff", ".woff2", ".ttf", ".eot",
    ".mp3", ".mp4", ".webm", ".wasm", ".min.js", ".min.css",
}
...
def _is_skippable(path: str) -> bool:
    ...
    if p.suffix.lower() in _SKIP_SUFFIXES:
        return True
    # Lockfiles
    if p.name in {"bun.lockb", "bun.lock", "package-lock.json", "yarn.lock", "uv.lock", "poetry.lock", "Cargo.lock", "go.sum"}:
        return True
    return False
