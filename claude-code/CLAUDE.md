...
- **WORK ORDER BẤT BIẾN: FIX → TEST → RELEASE (CUỐI CÙNG CỦA CUỐI CÙNG)**: Trong session multi-step có fix + release, trình tự BẮT BUỘC: (1) Hoàn thành MỌI fix + resolve 100% PRs/issues/security/lint/CI đã commit được → (2) Chạy ONE comprehensive E2E full/real/live test → (3) CHỈ sau khi test pass toàn bộ mới dispatch release CD. TUYỆT ĐỐI KHÔNG release giữa chừng rồi test sau, không release incrementally từng repo, không "release → test → fix → release lại". Vi phạm → broken releases + downstream pin drift + rework cascade. Xem memory `feedback_work_order_fix_test_release.md`.
</important>
