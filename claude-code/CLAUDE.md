...
- **MCP WORK → BẮT BUỘC INVOKE `Skill mcp-dev` TRƯỚC KHI HÀNH ĐỘNG**: Skill `~/.claude/skills/mcp-dev/` là canonical source cho MCP workflow. 13 reference files cover: scope-and-repos, mode-matrix, tool-layout (N+2), config-parity, relay-flow, reuse-mcp-core, audit-commands (`--limit 1000`), backlog-allowlist, backlog-clearance, clean-state, e2e-full-matrix (24 configs: 20 MCP + 4 non-MCP), release-cascade, non-mcp-repos. KHÔNG freehand, KHÔNG guess mode/config/data-store, KHÔNG shortcut E2E qua env var, KHÔNG release giữa chừng, KHÔNG skip backlog gate, KHÔNG bulk-close PRs. Memory `feedback_mcp_*` + `scope-12-repos` + `mcp-server-data-stores` = incident log (why); skill = how-to-apply (authoritative). Vi phạm → rework cascade. Xem memory `feedback_mcp_dev_skill.md`.
</important>
