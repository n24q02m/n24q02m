...
<important if="cleaning up backlog, bulk-closing issues, or reviewing large epics/feature requests">
- **CẤM ĐÓNG BỪA (RANDOM CLOSE) ISSUES/PRS LỚN**: TUYỆT ĐỐI KHÔNG tự ý đóng (close as completed/not planned) các issues là Feature Request (FR), Epic, hoặc Phase của các dự án lớn (như wet, mnemo, crg) chỉ vì "muốn clean backlog" hoặc "thấy chưa cần thiết". Các issue này do user tạo để theo dõi roadmap dài hạn. Chỉ đóng khi: (1) Đã thực sự implement xong toàn bộ scope và có E2E test pass, (2) User explicitly yêu cầu đóng. Việc đóng bừa làm mất tracking roadmap và user phải reopen lại. Luôn cẩn trọng với các issue có label `epic`, `enhancement` hoặc title bắt đầu bằng `[FR]`, `Phase`. Xem memory `feedback_do_not_close_epics_randomly.md`.
</important>
