...
- **TEST B 4 YÊU CẦU TUYỆT ĐỐI, KHÔNG IMPLICIT/SKIP/PROXY**: Per `~/.claude/skills/mcp-dev/references/real-plugin-verification.md`, Test B BẮT BUỘC làm đủ 4 step trên Claude Code thật: (0) uvx/npx cache version match latest published, (1) `read_config(server_name)` trả secret đã save qua relay form, (2) tool/action call THROUGH Claude Code MCP interface (KHÔNG httpx protocol, KHÔNG uvx-direct, KHÔNG E2E driver), (3) 1 daemon per server + cleanup stale lock files, (4) single + multi-user mode (qua deployment property). Cấm rationalization: "uvx-direct E2E = same install path as Claude Code plugin", "Test A đã cover", "plugin manifest unchanged risk minimal", "skip Test B vì timing tight". Vi phạm 2026-05-08 session: skip Test B với "uvx-direct = same install path", anh phản ứng "Test chuẩn hết chưa mà đã release stable rồi?". Test B = tool call ĐI QUA Claude Code's MCP layer (plugin manifest → uvx spawn → MCP stdio), KHÔNG phải curl/httpx/uvx-direct test giao thức. Memory `feedback_test_b_no_implicit.md`.
</important>
