...
- **ADMIN MERGE DEFAULT cho personal repo low-risk PR**: n24q02m/* + [CI green hoặc non-essential fail safe + diff reviewed evidence concrete + user admin] → `gh pr merge --admin --squash` ngay. CẤM propose A/B options "anh approve CLI / admin merge". CẤM "branch protection require review" — user admin dùng `--admin`. Chained 4-5 PR cùng type → batch merge + report SHA list. VẪN ASK khi: production-critical (KP main / web-core stable cascade), scope creep, shared/team repo, user explicit "đừng admin merge". Memory `feedback_admin_merge_default_low_risk.md`.
</important>
...
- **VERIFY CI GREEN AFTER EVERY MERGE/PUSH/DISPATCH**: Sau MỖI `gh pr merge`/`git push main`/`gh workflow run` → BẮT BUỘC `gh run list -R <repo> --branch main --limit 3` cùng response. `conclusion=="failure"` → STOP + investigate + fix root cause TRƯỚC batch tiếp theo. Cấm "merged N PRs, move on" rồi để main red. Cuối clear cycle: `for repo in <scope>; do gh run list -R n24q02m/$repo --branch main --limit 1 --jq '.[0].conclusion'; done` all=success. Memory `feedback_verify_ci_after_merge.md`.
</important>
...
- **MIGRATION PHASE 0 AUDIT BẮT BUỘC**: Trước commit Phase 1, execute Phase 0 + user approve: (1) inventory grep all n24q02m repos cho key/env var, (2) dependency map per key, (3) pagination/CLI silent-truncate check, (4) existing identity audit — KHÔNG reuse token "gần đó", (5) test 1 service end-to-end + evidence, (6) spec/plan checkpoint. Iterate-patch sau migration = customer outage. Memory `feedback_migration_audit_first.md`.
</important>
...
- **~/.claude/ SYNC PUBLIC `n24q02m/n24q02m`**: TUYỆT ĐỐI KHÔNG inline secret/API key/token/email cá nhân/IP/hostname/account ID/OAuth client ID/DB URL/skret SSM full path. Sync GitHub → leak permanent. Reference memory entry name only (`xem memory feedback_xxx.md`); cred values ở `<project>/memory/*.md` (local, không sync). Audit trước save: có value cụ thể? → replace placeholder. Memory `feedback_claude_md_public_sync.md`.
</important>
...
- **CẤM "ĐỂ FIX SAU"**: bug/security trong session đang chạy → fix NGAY. KHÔNG note "fix sau"/"Phase M.2"/"next cycle"/"tracking issue cho later". Áp dụng (a) security issues (b) broken user flows (c) data loss risks (d) bugs đã xác định root cause. Chỉ defer nếu cần infra change/coordination ≥2 engineers + user approve. Memory `feedback_never_defer_tasks.md`.
</important>
...
<important if="cần tìm scope repo multi-repo audit/cleanup/release/rollout, hoặc sắp hardcode list tên repo vào spec/plan/rule">
- **REPO SCOPE = 2 STARS LISTS (Productions + Scripts), KHÔNG hardcode count**: Fetch qua GraphQL `viewer.lists` (REST/web trả 404 cho Stars). Detail + recipe: `~/.claude/skills/infra-devops/references/repo-structure.md` "Tier 1/Tier 2". Memory `scope-12-repos.md`.
</important>
...
- **CF TOKEN 3-CATEGORY FRAMEWORK**: phân biệt project / infra / dev-full-access token TRƯỚC khi xử lý. NEVER ask user paste CF token khi token đã trong skret (vi phạm 2026-05-08: 5 pushback). Detail (3 categories + retrieval recipe + verify endpoint pitfall): `~/.claude/skills/infra-devops/references/cf-token-framework.md`. Memory `feedback_cf_global_token.md` + `feedback_cf_token_verify_pitfall.md`.
</important>

<important if="editing ANY file under ~/.claude/ (CLAUDE.md, skills/*) — sau mỗi save check size">
- **CLAUDE.md SIZE <40KB BẮT BUỘC**: Sau MỖI edit `~/.claude/CLAUDE.md` BẮT BUỘC `wc -c ~/.claude/CLAUDE.md`. Nếu ≥40KB (40960 bytes) → DISTILL ngay: pick 1-2 longest blocks, replace inline detail bằng pointer `Detail: ~/.claude/skills/<...>.md. Memory: <...>`. KHÔNG để rule rot vì file quá dài → context window pollution + truncation. Đo > limit = TODO mặc định trong cùng response, không defer.
</important>
