...
<important if="just merged a PR, pushed a commit to main, dispatched a CD workflow, OR claimed 'shipped'/'merged'/'released' for any repo">
- **VERIFY CI GREEN AFTER EVERY MERGE/PUSH/DISPATCH, KHÔNG batch-claim done**: Sau MỖI `gh pr merge`/`git push origin main`/`gh workflow run cd.yml` → BẮT BUỘC `gh run list -R <repo> --branch main --limit 3 --json conclusion,name,databaseId --jq '.[]'` trong cùng response. Nếu `conclusion == "failure"` → STOP + investigate fail log + fix root cause TRƯỚC khi tiếp tục batch tiếp theo. Cấm "merged 7 PRs, full suite passed locally, move on" rồi để main red 4 tiếng không biết. Vi phạm 2026-05-08 session: clear backlog 6 repos, merge 96 PRs, KHÔNG check CI sau push → mcp-core/web-core/qwen3-embed/crg main red, user phải nhắc "ci fail, đỏ lòm như vậy từ bao giờ mà không chịu fix gì cả??". Sau audit script Phase 0 chạy END của clear cycle: chạy thêm `for repo in <scope>; do gh run list -R n24q02m/$repo --branch main --limit 1 --json conclusion --jq '.[0].conclusion'; done` → all should be `success` hoặc rolled back. Memory `feedback_verify_ci_after_merge.md`.
</important>
...
- **TEST B 4 YÊU CẦU TUYỆT ĐỐI + MATRIX-IN-SETTINGS, KHÔNG IMPLICIT/SKIP/PROXY/MARKETPLACE-INSTALL**: Per `~/.claude/skills/mcp-dev/references/real-plugin-verification.md` + `test-b-matrix-in-settings.md`, Test B BẮT BUỘC làm đủ 4 step trên Claude Code thật: (0) uvx/npx cache version match latest published, (1) `read_config(server_name)` trả secret đã save qua relay form, (2) tool/action call THROUGH Claude Code MCP interface = invoke `mcp__plugin_<plugin>_<server>__<tool>` (KHÔNG httpx protocol, KHÔNG uvx-direct, KHÔNG E2E driver), (3) 1 daemon per server + cleanup stale lock files, (4) single + multi-user mode (qua deployment property). **Test B fixture = matrix-in-settings**: 8 plugin × 3 method (stdio uvx + HTTP Docker + stdio Docker) = 24 entries TRỰC TIẾP trong `~/.claude.json mcpServers`, 1 CC restart loads tất cả, KHÔNG qua marketplace install (stale cache + version drift + env block reset + /reload-plugins unreliable). Memory `feedback_test_b_no_implicit.md` + `feedback_drop_t2_for_test_b.md` + `feedback_settings_matrix_skip_plugin.md`.
</important>
...
<important if="user request scope đơn — fix/sửa/audit/cải tiến 1 thứ cụ thể named (vd 'fix qship', 'remove paperclip', 'restart telegram')">
- **SCOPED CHANGE BẮT BUỘC**: CHỈ động files/services/configs explicit named trong scope. Discovery issue Y/Z liền kề → ghi vào response cuối heading "Out of scope, anh muốn fix tiếp?" rồi STOP. KHÔNG drive-by refactor (rename/reformat/restructure adjacent), KHÔNG cleanup unrelated, KHÔNG reuse identity/token "có sẵn gần đó" cho purpose mới. Cleanup blocker bắt buộc fix CHỈ khi blocking immediate task (vd remove compose include khi folder đã xóa khiến `make up` fail). Multi-service migration → invoke `superpowers:writing-plans`. Vi phạm session 4f482ae2 (2026-05-06): user "fix qship" → em pivot sang remove 5 services + build alerting + reshape VM + watchdog setup = 1 ngày thay 30 phút. Memory `feedback_scoped_change_discipline.md`.
</important>
<important if="migration ảnh hưởng ≥3 service HOẶC touch ≥10 keys/configs/env vars (vd skret migration, transport swap, infra restructure, multi-repo bump, alert/identity wire-up)">
- **MIGRATION PHASE 0 AUDIT BẮT BUỘC**: Trước khi commit Phase 1 implementation, execute Phase 0 audit + user approve: (1) inventory grep ALL n24q02m repos cho key/env var liên quan (`gh repo list --limit 1000` + grep), (2) dependency map per key (producer/consumer/injection path), (3) pagination/CLI silent-truncate check (skret >56 keys, gh default 30, AWS describe-parameters defaults), (4) existing identity audit (bot/user/role) — `getMe` per TG bot token, `aws iam get-user`, KHÔNG reuse token "gần đó", (5) test 1 representative service end-to-end với evidence, (6) spec/plan checkpoint với audit output. Iterate-discover-patch sau migration = customer outage. Vi phạm: skret migration 2026-04 → 3-day customer outage qship 2026-05-03 (109 × HTTP 503) vì PRIMARY_PARTNER_BASE_URL bỏ sót; watchdog 2026-05-07 wire MCP bot thay vì alert bot. Memory `feedback_migration_audit_first.md`.
</important>
...
<important if="cần CF API token cho bất kỳ Cloudflare op nào — Pages deploy, custom domain claim, DNS records edit, zone config, R2, Workers, Tunnel, account-wide audit, dashboard-replacement automation">
- **CF TOKEN 3-CATEGORY FRAMEWORK** — phân biệt rõ trước khi xử lý, KHÔNG conflate, KHÔNG ASK user khi token đã trong skret:
  - **Project token** (per app): `/<App>/<env>/<APP>_CF_PAGES_API_TOKEN` — Aiora/KnowledgePrism/QuikShipping/skret/mcp project Pages CD only
  - **Infra token**: `/oci-vm-{infra,prod}/{prd,prod}/CF_R2_*` + `CF_TUNNEL_TOKEN` — VM ops (R2, Tunnel) only
  - **Full-access dev token** (label "Edit almost everything resource"): `/n24q02m/dev/CF_DEV_TOKEN` (ap-southeast-1) — Pages + Zone + DNS + Workers + R2 cross all n24q02m zones; for dashboard-replacement ops em làm thay user
- **Retrieval**: `MSYS_NO_PATHCONV=1 AWS_REGION=ap-southeast-1 aws ssm get-parameter --name /n24q02m/dev/CF_DEV_TOKEN --with-decryption --query 'Parameter.Value' --output text`. Skret CLI có bug retrieving this entry; aws ssm direct exception cho retrieval, NOT for write ops.
- **Verify validity**: hit real endpoint `curl /zones?name=n24q02m.com -H "Authorization: Bearer $TOKEN"`, NEVER `/user/tokens/verify` (lies for scoped tokens per `feedback_cf_token_verify_pitfall.md`).
- **NEVER ask user paste CF token** khi token đã trong skret. Vi phạm 2026-05-08: 5 user pushback liên tiếp vì confused 3 categories. Memory `feedback_cf_global_token.md` (đã rewrite 2026-05-08 với category framework + retrieval recipe).
</important>
